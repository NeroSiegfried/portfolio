...
/**
 * Renders the content inside a device screen area with progressive quality loading.
 *
 * Tiers (generated by capture-screenshots.mjs):
 *   lo  → {id}-{device}-lo.jpg  ~25 % dimensions, JPEG 55 % — loaded immediately
 *   md  → {id}-{device}-md.jpg  ~50 % dimensions, JPEG 75 % — good for 1× displays
 *   hi  → {id}-{device}.png     full resolution PNG          — for 2× / retina
 *
 * After the page `load` event fires, each project upgrades in order of its
 * visual position (projectPosition * 400 ms stagger), and the final tier is
 * chosen from the network speed + device pixel ratio:
 *   save-data / 2G → stay at lo
 *   3G             → upgrade to md
 *   fast / no API  → upgrade to md then hi (hi only when DPR ≥ 2)
 */
function ScreenContent({
  ...
  const hiUrl = url ?? (fallbackLiveUrl && projectId != null
    ? `/projects/${projectId}-${deviceType}.png`
    : null)

  // Only apply progressive loading to the auto-generated /projects/*.png files.
  ...
  useEffect(() => {
    ...
    const getTarget = (): "md" | "hi" | null => {
      ...
      if (et === "3g") return "md"
      // Fast connection or API unavailable: use DPR to pick quality ceiling
      return (window.devicePixelRatio ?? 1) >= 2 ? "hi" : "md"
    }

    const upgrade = async () => {
      // Stagger upgrades so topmost visible project loads first
  ...
  return (
    <div style={wrapStyle}>
      ...
      <img
        ...
        style={imgStyle}
        onError={(e) => {
          // lo/md tier missing (not yet generated) → fall back to full PNG
...
function WebPreview({ project, projectPosition }: { project: Project; projectPosition: number }) {
